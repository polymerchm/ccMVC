...
#--- ---playback constants
	...
	def noChords(self):
		numChordsTextView.text = ''
		chordNumTextView.text = ''
		...
		if self.tuning:
			...
			if (self.fingerings and self.cc_mode == 'C') or (self.cc_mode == 'P' and self.cFingering):
				...
				if not self.showChordScale:
					...
					setDisplays("{}".format(numChord),'of',"{}".format(chordNum))
					middle_field.text = 'of'
					if len(Fingering) == 3:
						fingering,chordTones,fretPositions= Fingering
					else:
						fingering,chordTones,fretPositions,_ = Fingering
					ui.set_color('red')
...
class SettingListDelegate(object):
	...
	def __init__(self):
		...
		pub.subscribe(self.setDefaultSettings,'setdefaultsettings')
		
	
	def setDefaultSettings(self):
		pub.sendMessage('changesettings',settings=self.items[0])
...
class ProgressionEditor(TVTools,ui.View):	
	def did_load(self):
		self.progSeq = self['label_prog_seq']
		self.progChord = self['label_prog_chord']
		self.nextButton = self['button_editprog_next']
		self.nextButton.action = self.onNext
		self.undoButton =self['button_editprog_undo']
		self.undoButton.action = self.onUndo
		self.clearButton = self['button_editprog_clear']
		self.clearButton.action = self.onClear
		self.doneButton = self['button_editprog_done']
		self.doneButton.action = self.onDone
		self.cancelButton = self['button_editprog_cancel']
		self.cancelButton.action = self.onCancel
		self.chordType = self['tableview_editprog_types']
		self.chordType.data_source = self.chordType.delegate = self
		self.items = ccc['CHORD_LIST_CLEAN']
		self.currentType = 'maj' # default
		self.sequenceString = ''
		self.sequenceNames = []
		self.sequenceTuples = []
		self.rootName = ''
		...
		margin = 20
		width -= margin*2 # leave a buffer on each end
		width -= margin*5 # account for interbutton spacing
		btnWidth = math.floor(width/6)
		height = 30
		for number in range(12):
			button = ui.Button()
			button.name = 'button_scale_tone_{}'.format(number)
			button.title = ccc['PROGRESSION_ROOTS'][number]
			button.border_width = 1
			button.border_color = '#000000'
			button.background_color = '#FFFFFF'
			self.add_subview(button)
			row,col = divmod(number,6)
			x = margin + (btnWidth + margin)*col
			y = row*50 + 20
			button.frame = (x,y,btnWidth,height)
			button.action = self.onRootButton
		
	def onEdit(self,button): #enable editor
		mainViewShield.conceal()
		self.hidden = False
		self.bring_to_front()
	...
	def onNext(self,button):
		if self.rootName and self.chordType:
			...
			console.hud_alert('need a root and chord type')
			return
		self.sequenceString = ''
		for seq in self.sequenceNames:
			self.sequenceString += seq
	...
	def tableview_delete(self, tableview, section, row):
		# Called when the user confirms deletion of the given row.
		self.currentNumLines -=1 # see above regarding hte "syncing"
		self.currentType.delete_rows((row,)) # this animates the deletion  could also 'tableview.reload_data()'
		del self.items[row]
...
class ProgDisplay(TVTools,object):
	def __init__(self,delegator):
		...
		self.frame = self._Delegator.frame
		self._Delegator.context_size = (self._Delegator.width,400)
		self.maxRows = 20
		...
		pub.subscribe(self.clearProg,'clearprog')
	...
	def updateProgressionChord(self,chord=None):
		if chord > self.maxRows-1:
			self._Delegator.content_offset = (0,(chord-self.maxRows+1)*self._Delegator.row_height)
		else:
			self._Delegator.content_offset = (0,0)
		self.setTableViewItemsRow(chord)
	...
	def updateProgressionList(self,progFingerings=None, progChords=None):	
		...
		rowHeight = self._Delegator.row_height
		newHeight = min(len(self.items),self.maxRows)*rowHeight #scolls after that
		self._Delegator.frame = (x,y,width,newHeight)
		self._Delegator.content_size = (width,rowHeight*(len(self.items)+5))
...
class Progr(TVTools,object):
	...
	def progListEdit(self,button):
		if self.editing:
			...
			self._Delegator.bring_to_front()
			for item in self.items:
				item['accessory_type'] = 'none'
	...
	def tableview_did_select(self,tableView,section,row):  		
		...
		if row == 0:
			...
			if len(self._progFingerings[0]) == 0:
				print("did not find any fingerings")
				print("row = ",row)
				print("progchords = ",self._progChords)
				
				raise RunTimeError("no fingerings")
			else:
...
def playProgression(button):	
	if not model._ProgFingerings or len(model._ProgFingerings[0]) == 0:
		return
	if os.path.exists('waves'):
		if not model._InstrumentOctave:
			return
...
def toggle_mode(sender,row):
	...
	buttonEditChord = mainView['button_edit_chord']
	buttonEditChord.action = None
	if newmode == 'C': # special stuff for identify
		buttonEditChord.title = 'type'
	elif newmode == 'S':
		buttonEditChord.title = 'mode'
		# set default and C major
	elif newmode == 'I':
		buttonEditChord.title = ''
		...
		mainView['tableview_prog_display'].hidden = True
		buttonEditChord.title = 'new progr'
		buttonEditChord.action = progEditor.onEdit
...
def setDisplays(top,middle,bottom):
	if bottom:
		chordNumTextView.text = bottom
		chordNumTextView.hidden = False
	else:
		chordNumTextView.hidden = True
		

	if middle_field:
		middle_field.text = middle
		middle_field.hidden = False
	else:
		middle_field.hidden = True

	if top:
		numChordsTextView.text  = top
		numChordsTextView.hidden = False
	else:
...
if __name__ == "__main__":
	...
	mainView['slider_arp'].action = onSliderSpeed
	mainView['slider_volume'].action = onSliderVolume
	...
	progEditor = mainView['view_prog_editor']
	progEditor.hidden = True
